...
const smsMods = () => { return `\`ESTE COMANDO SÓLO MODERADORES Y MI CREADOR LO PUEDE USAR.\`` }//MODERADORES
const smsPremium = () => { return `\`ESTE COMANDO SÓLO ESTA DISPONIBLE PARA USUARIOS PREMIUM Y MI CREADOR.\`` }//USUARIOS PREMIUM
...
const smsUnreg = () => { return `\`NECESITAS ESTAR REGISTRADO PARA USAR ESTE COMANDO.\`` }//VERIFICAR
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DE SerberoX 🌟`}
...
const smsLista7 = () => { return `🐈 INSTALAR SerberoX 🐈`}
...
//_allantilink.js
const smsTextoYT = () => { return 'SerberoX'} 
const smsApagar = () => { return '❌ DESACTIVAR'} 
const smsEncender = () => { return '✅ ACTIVAR'} 
const smsEnlaceTik = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙏𝙄𝙆𝙏𝙊𝙆 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
const smsEnlaceYt = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙔𝙊𝙐𝙏𝙐𝘽𝙀 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
const smsEnlaceTel = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙏𝙀𝙇𝙀𝙂𝙍𝘼𝙈 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
const smsEnlaceFb = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
const smsEnlaceIg = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙄𝙉𝙎𝙏𝘼𝙂𝙍𝘼𝙈 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
const smsEnlaceTw = () => { return `𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝘿𝙀 𝙏𝙒𝙄𝙏𝙏𝙀𝙍 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊`}
...
//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeGB['smsAvisoAG']()}𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊\n\n𝙋𝙍𝙊𝘾𝙀𝘿𝙊 𝘼 𝙀𝙇𝙄𝙈𝙄𝙉𝘼𝙍𝙏𝙀`}
...
const eGataCoins = () => { return 'X-Coins' }
const eGataTickers = () => { return '🎫 X Tickers' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar el bot`}
...
const smsOnlyOwner = () => { return `☕ ONLY OWNER` }
const smsOnlyOwner2 = () => { return `Menciones .tagall y ht`}
const smsAntitoc1 = () => { return `☢️ ANTI TÓXICOS`}
const smsAntitoc2 = () => { return `Enviar advertencias a las personas que insulten`}
...
const smsAntiEE1 = () => { return `🔗 ANTI ENLACES 2 FULL`}
const smsAntiEE2 = () => { return `Eliminar TODOS los enlaces que se detecten`}
const smsAntiTT1 = () => { return `⚠️🔗 ANTI TIKTOK`}
const smsAntiTT2 = () => { return `Eliminar enlaces de TikTok`}
const smsAntiYT1 = () => { return `⚠️🔗 ANTI YOUTUBE`}
const smsAntiYT2 = () => { return `Eliminar enlaces de YouTube`}
const smsAntiTEL1 = () => { return `⚠️🔗 ANTI TELEGRAM`}
const smsAntiTEL2 = () => { return `Eliminar enlaces de Telegram`}
const smsAntiFB1 = () => { return `⚠️🔗 ANTI FACEBOOK`}
const smsAntiFB2 = () => { return `Eliminar enlaces de Facebbok`}
const smsAntiIG1 = () => { return `⚠️🔗 ANTI INSTAGRAM`}
const smsAntiIG2 = () => { return `Eliminar enlaces de Instagram`}
const smsAntiTW1 = () => { return `⚠️🔗 ANTI TWITTER `}
const smsAntiTW2 = () => { return `Eliminar enlaces de Twitter`}
...
const smsConfi10 = () => { return `*⚙️CENTRO DE CONFIGURACIÓN⚙️*`}
...
//propietario(a.js
const smsPropban1 = (usedPrefix, command, bot) => { return `*ETIQUETE A ALGUIEN PARA BANEAR.*`}
const smsPropban2 = (bot) => { return `*@${bot} NO PUEDE SER BANEADO CON ESTE COMANDO*`}
const smsPropban3 = (ownerNumber) => { return `*NO PUEDO BANEAR AL OWNER @${ownerNumber} DEL BOT*`}
const smsPropban4 = (number) => { return `*NO ES NECESARIO VOLVER A BANEAR A @${number} SI YA LO ESTÁ*`}
const smsPropban5 = () => { return `𝙀𝙇/𝙇𝘼 𝙐𝙎𝙐𝘼𝙍𝙄𝙊(𝘼) 𝙁𝙐𝙀 𝘽𝘼𝙉𝙀𝘼𝘿𝙊(𝘼)`}
...
const smsPropban7 = (usedPrefix, command, number) => { return `*SURGIÓ UN ERROR, PUEDE SER QUE EL USUARIO NO ESTE EN MI BASE DE DATOS INTENTE ESCRIBIR ${usedPrefix + command} ${number}*\n\`\`\`SI EL ERROR CONTINÚA REPORTE ESTE COMANDO\`\`\``}

//jadibot-serbot.js
const smsIniJadi = () => { return `*Codigo QR.*`}
const smsIniJadi2 = () => { return `*Generando codigo de vinculación..*`}
...
const smsJBDel = () => { return `*USE ESTE COMANDO AL BOT PRINCIPAL*`}
const smsJBAdios = () => { return `*TE VOY A EXTRAÑAR ${global.packname} CHAOO!! 🥹*`}
const smsJBCerrarS = () => { return `*HA CERRADO SESIÓN Y BORRADO TODO RASTRO*`}
const smsJBErr = () => { return `*HA CERRADO SESIÓN COMO SUB BOT*`}

//comandos+18-adult.js
const smsContAdult = () => { return `*LOS COMANDOS 🔞 ESTÁN DESACTIVADOS*`}
