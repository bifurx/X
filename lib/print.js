...
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Usuario:' : 'User:')} ${chalk.yellow.bold('%s')} 
┃ ❖ %s
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Tipo de mensaje:' : 'Type of message')} ${chalk.white('[%s]')}
╰━━━━━━━━━━━━━━𖡼`.trim(),

me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' 【𝗦𝗨𝗕 𝗕𝗢𝗧】'}`,
sender,
(m.chat.includes("@g.us") ? `${chalk.white.bold(mid.idioma_code === 'es' ? 'Grupo:' : 'Group:')} ${chalk.blue.bold(`${m.chat} ~${chat}`)}` : `${chalk.white.bold(mid.idioma_code === 'es' ? 'Privado:' : 'Private')} ${chalk.green.bold(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
